  ...
  useEffect(() => {
    ...
    const fsSource = `
      ...
      void main() {
        ...
        // White background, subtle blue topographic lines
        vec3 baseColor   = vec3(1.0,  1.0,  1.0);   // pure white
        vec3 accentColor = vec3(0.96, 0.97, 1.0);   // barely-there blue tint between lines
        vec3 lineColor   = vec3(0.60, 0.75, 0.95);  // soft blue lines

        vec3 finalColor = mix(baseColor, accentColor, n * 0.06);
        finalColor += topoLine * lineColor * depth * 0.55;
    ...
    const render = (time) => {
      // Use container dimensions so shader resolution matches actual display size
